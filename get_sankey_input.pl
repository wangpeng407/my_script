...
while(<IN>){
  ...
  for my $i (0..$#r_taxa-1){ 
    if($r_taxa[$i+1] && $r_taxa[$i]){
      ...
      #print $r_taxa[$i+1] . "(" . format_digit($taxa_abund{$r_taxa[$i+1]}) . ")", "\t", $r_taxa[$i] . "(" . format_digit($taxa_abund{$r_taxa[$i]}) . ")", "\t", $taxa_abund{$r_taxa[$i]}, "\n";
    }
  }
}
close IN;
...
my $link_sankey = $outdir . '/sankey.link.txt';
my $node_sankey = $outdir . '/sankey.node.txt';
...
#     my @r_taxa = reverse(@taxa);
#     for my $i (0..$#r_taxa){
#       if($r_taxa[$i] && $r_taxa[$i+1]){
#         $taxa1_taxa2{$r_taxa[$i]} = $r_taxa[$i+1];
#       }
#     }
#   }
#   close IN;
